...
@section('page_heading')
  <div class="w-full flex items-center justify-between">
    {{ __('Brands') }}
    <form method="POST" action="{{ route('brands.export') }}">
      @csrf
      <a href="{{ route('brands.export') }}" onclick="event.preventDefault(); this.closest('form').submit();"
...
@section('content')
  <div
    ...
    <form id="iForm" method="post" action="{{ route('brands.import') }}" enctype="multipart/form-data">
      @csrf
      <p class="text-gray-600">{{ __('Please select the excel file to import brands.') }}</p>
      <div class="pt-5">
          ...
          <div id="dropzone" class="flex items-center justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md"
            ...
            <div class="text-center">
              ...
              <div class="text-gray-700">
                <div>{{ __('Excel file should have name, code, order, details & photo columns and size up to 2MB.') }}</div>
                <div>{{ __('You must fill the name and code columns.') }} {{ __('order is the sorting number.') }}
