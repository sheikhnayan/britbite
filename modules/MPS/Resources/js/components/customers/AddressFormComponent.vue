      ...
      <Row :gutter="16">
        <Col :sm="24" :md="24" :lg="24">
          ...
          <FormItem class="mb0">
            ...
            <Button v-if="form.id" type="error" @click="deleteAddress()" style="margin-left:8px;float:right">{{ $t('delete') }}</Button>
          </FormItem>
        </Col>
      </Row>
    </Form>
...
export default {
  ...
  methods: {
    ...
    deleteAddress() {
      this.$http
        .delete('app/customers/delete_address/' + this.form.id)
        .then(res => {
          if (res.data.success) {
            this.$Notice.destroy();
            this.$Notice.success({ title: this.$tc('address') + ' ' + this.$t('deleted'), desc: this.$t('record_deleted') });
