<template>
  <div class="side-menu-wrapper">
    ...
    >
      <template v-for="(item, ii) in menuList">
        <template v-if="item.children && item.children.length === 1">
          <side-menu-item
            :parent-item="item"
            v-if="showChildren(item)"
            :key="`m-${(item.name ? item.name : item.meta.mName) || ii + 1}`"
          ></side-menu-item>
          <menu-item
            v-else
            :name="getNameOrHref(item, true)"
            :key="`menu-${(item.children[0].name ? item.children[0].name.name : item.children[0].name.meta.mName) || ii + 1}`"
          >
        ...
        <template v-else>
          <side-menu-item
            ...
            :key="`m-${(item.name ? item.name : item.meta.mName) || ii + 1}`"
          ></side-menu-item>
          <menu-item v-else :name="getNameOrHref(item)" :key="`menu-${(item.name ? item.name : item.meta.mName) || ii + 1}`">
            <Icon :type="item.icon || ''" />
