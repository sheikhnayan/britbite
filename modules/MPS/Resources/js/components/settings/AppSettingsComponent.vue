      ...
      <div slot="header">
        <h3>
          ...
          <Button
            type="info"
            size="small"
            @click="scale_barcode_guide = true"
            style="margin-top: -0.25rem; float: right; margin-right: 2rem"
            >{{ $t('guide') }}</Button
          >
        </h3>
      </div>
...
export default {
  ...
  methods: {
    getWeekDayString(day) {
      return new Date(this.$moment().day(day).format('YYYY-MM-DD')).toLocaleString('default', { weekday: 'long' });
    },
    fetch() {
      this.$http
        .get('app/settings')
