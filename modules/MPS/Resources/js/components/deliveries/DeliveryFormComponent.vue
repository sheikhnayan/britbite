            ...
            <Col :sm="24" :md="24" :lg="24">
              ...
              <Row :gutter="16">
                <Col :sm="24" :md="12" :lg="12">
                  ...
                  <FormItem prop="delivered_by" :label="$tc('delivered_by')" :error="errors.form.delivered_by | a2s">
                    <Input v-model="form.delivered_by" />
                  </FormItem>
                  <FormItem prop="delivered_at" :label="$tc('delivered_at')" :error="errors.form.delivered_at | a2s">
                    <DatePicker type="datetime" v-model="form.delivered_at" format="yyyy-MM-dd HH:mm" style="width: 100%;" />
                  </FormItem>
                </Col>
                <Col :sm="24" :md="12" :lg="12">
                  ...
                  <FormItem prop="received_by" :label="$tc('received_by')" :error="errors.form.received_by | a2s">
...
export default {
  ...
  data() {
    const deliveredBy = (rule, value, callback) => {
      if (this.form.status == 'delivered' && (!value || value == '')) {
        callback(new Error(this.$t('field_is_required', { field: this.$t('delivered_by') })));
      } else {
        callback();
      }
    };
    const receivedBy = (rule, value, callback) => {
      if (this.form.status == 'delivered' && (!value || value == '')) {
        callback(new Error(this.$t('field_is_required', { field: this.$t('received_by') })));
      } else {
        callback();
      }
    };
    const deliveredAt = (rule, value, callback) => {
      if (this.form.status == 'delivered' && (!value || value == '')) {
        callback(new Error(this.$t('field_is_required', { field: this.$t('delivered_at') })));
      } else {
        callback();
      }
    };
        ...
        delivered_by: [
          {
            trigger: 'blur',
            required: true,
            validator: deliveredBy,
            message: this.$t('field_is_required', { field: this.$t('delivered_by') }),
          },
        ],
        received_by: [
          {
