    ...
    public static function scopeMyPayments($query)
    {
        ...
        if (!$user->hasRole(['admin', 'super'])) {
            if ($user->hasRole('customer')) {
                ...
                return $query->where('payable_id', $user->supplier_id)->where('payable_type', 'Modules\MPS\Models\Vendor');
            }
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope('mine', function (Builder $builder) {
            ...
            if ($user && !$user->hasRole('super') && !$user->view_all) {
                if ($user->customer_id || $user->hasRole('customer')) {
                    return $builder->where('payable_id', $user->customer_id)->where('payable_type', 'Modules\MPS\Models\Customer');
                } elseif ($user->supplier_id || $user->hasRole('supplier')) {
                    return $builder->where('payable_id', $user->supplier_id)->where('payable_type', 'Modules\MPS\Models\Supplier');
                }
