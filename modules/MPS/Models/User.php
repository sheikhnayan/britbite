...
class User extends AppUser implements HasMedia
{
    ...
    protected $fillable = [
        'name', 'email', 'phone', 'active', 'username', 'password', 'customer_id', 'supplier_id',
        'location_id', 'view_all', 'edit_all', 'bulk_actions', 'extra_attributes', 'avatar',  'employee',
    ];
    ...
    public function del()
    {
        if ($this->sales()->exists() || $this->purchases()->exists() || $this->incomes()->exists() || $this->expenses()->exists() || $this->quotations()->exists() || $this->returnOrders()->exists() || $this->timeClocks()->exists() || $this->addresses()->exists()) {
            return false;
        }
